<template>
  <section>
    ...
    <div class="flex">
      <h2 class="title-itinerary">Mes itinéraires</h2>
    ...
    <div class="pos-btn-itinerary">
      <button
        class="button"
        @click="this.createFormVisible = !this.createFormVisible"
      >
        Nouvel itinéraire
      </button>
    </div>
...
export default {
  ...
  methods: {
    async handleItinerarySubmitted(itinerarySubmitted) {
      if (
        itinerarySubmitted.name.length === 0 ||
        itinerarySubmitted.checkpoints[0].length === 0
      ) {
        errorToast(
          "Vous devez indiquer des informations valides pour cet itinéraire"
        );
        return;
      }
      const { status, itinerary } = await ItineraryService.create(
      ...
      if (status === 201) {
        this.$store.commit("setUser", {
          ...
          itineraries: [...this.$store.state.user.itineraries, itinerary],
        });
    ...
    handleItineraryEdited(itineraryEdited) {
      ...
      const index = copy.indexOf(
        this.$store.state.user.itineraries.find(
          (it) => it.id === itineraryEdited.id
        )
      );
    ...
    handleItineraryDeleted(deleted) {
      this.$store.commit("setUser", {
        ...this.$store.state.user,
        itineraries: this.$store.state.user.itineraries.filter(
          (it) => it.id !== deleted.id
        ),
      });
    },
